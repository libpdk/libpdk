...
#define SET(n) \
   (ctx->block[(n)] = \
   (pdk::puint32)ptr[(n) * 4] | \
   ((pdk::puint32)ptr[(n) * 4 + 1] << 8) | \
   ((pdk::puint32)ptr[(n) * 4 + 2] << 16) | \
   ((pdk::puint32)ptr[(n) * 4 + 3] << 24))
#define GET(n) \
   (ctx->block[(n)])
#endif
...
static const unsigned char *body(struct md4_context *ctx, const unsigned char *data, size_t size)
{
   ...
   pdk::puint32 a, b, c, d;
   pdk::puint32 saved_a, saved_b, saved_c, saved_d;
...
{
   /* @UNSAFE */
   pdk::puint32 saved_lo;
   unsigned long used, free;
   
   saved_lo = ctx->lo;
   if ((ctx->lo = (saved_lo + size) & 0x1fffffff) < saved_lo)
      ctx->hi++;
   ctx->hi += (pdk::puint32)(size >> 29);
