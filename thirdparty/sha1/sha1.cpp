...
{
   pdk::puint8  bytes[64];
   pdk::puint32 words[16];
} Sha1Chunk;

static inline pdk::puint32 rol32(pdk::puint32 value, unsigned int shift)
{
#ifdef PDK_CC_MSVC
...
static inline pdk::puint32 sha1Word(Sha1Chunk *chunk, const uint position)
{
...
static inline void sha1Round0(Sha1Chunk *chunk, const uint position,
                              pdk::puint32 &v, pdk::puint32 &w, pdk::puint32 &x, pdk::puint32 &y, pdk::puint32 &z)
{
...
static inline void sha1ProcessChunk(Sha1State *state, const unsigned char *buffer)
{
   ...
   pdk::puint32 a = state->h0;
   pdk::puint32 b = state->h1;
   pdk::puint32 c = state->h2;
   pdk::puint32 d = state->h3;
   pdk::puint32 e = state->h4;
   
   pdk::puint8 chunkBuffer[64];
   memcpy(chunkBuffer, buffer, 64);
   
   Sha1Chunk *chunk = reinterpret_cast<Sha1Chunk*>(&chunkBuffer);
   
   for (int i = 0; i < 16; ++i)
      chunk->words[i] = pdk::from_big_endian(chunk->words[i]);
...
static inline void sha1Update(Sha1State *state, const unsigned char *data, pdk::pint64 len)
{
   pdk::puint32 rest = static_cast<pdk::puint32>(state->messageSize & PDK_UINT64_C(63));
   
   pdk::puint64 availableData = static_cast<pdk::puint64>(len) + static_cast<pdk::puint64>(rest);
   state->messageSize += len;
   
   if (availableData < PDK_UINT64_C(64)) {
      memcpy(&state->buffer[rest], &data[0], len);
      
   } else {
      pdk::pint64 i = static_cast<pdk::pint64>(64 - rest);
      memcpy(&state->buffer[rest], &data[0], static_cast<pdk::pint32>(i));
      sha1ProcessChunk(state, state->buffer);
      
      pdk::pint64 lastI = len - ((len + rest) & PDK_INT64_C(63));
      for( ; i < lastI; i += 64)
...
{
   pdk::puint64 messageSize = state->messageSize;
   unsigned char sizeInBits[8];
   pdk::to_big_endian(messageSize << 3, sizeInBits);
...
{
   pdk::to_big_endian(state->h0, buffer);
   pdk::to_big_endian(state->h1, buffer + 4);
   pdk::to_big_endian(state->h2, buffer + 8);
   pdk::to_big_endian(state->h3, buffer + 12);
   pdk::to_big_endian(state->h4, buffer + 16);
}
